#!/usr/bin/env python3
"""Single-threaded HTTP server (for comparison). Sleeps ~1s per request.

Usage:
  python server_single.py <root_directory> [--host 0.0.0.0] [--port 8081]
"""
import os
import sys
import socket
...
PORT_DEFAULT = 8081
SERVER_NAME = "MinimalPythonHTTP-ST/1.0"
...
def directory_listing(path, req_path):
    ...
    for name in items:
        ...
        href = display
        # Build relative key like '/name' or '/sub/name'
        if disp_path in ('', '/'):
            rel = '/' + name.lstrip('/')
        else:
...
def handle_request(root: str, raw_request: bytes) -> bytes:
    ...
    # single-threaded: safe to update without lock, but use lock anyway to be consistent
    with COUNTS_LOCK:
...
def serve(root, host, port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.bind((host, port))
        s.listen(1)
        print(f"[single] Serving {root} on http://{host}:{port}")
        while True:
            conn, addr = s.accept()
            with conn:
                try:
...
def parse_args(argv):
    ...
    while i < len(argv):
        ...
        i += 1
    if not root:
        print('Usage: server_single.py <root> [--host HOST] [--port PORT]', file=sys.stderr)
        sys.exit(1)
