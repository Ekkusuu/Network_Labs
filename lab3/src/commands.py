...
async def look(board: Board, player_id: str) -> str:
    ...
    :param player_id: ID of player looking at the board; 
                     must be a nonempty string of alphanumeric or underscore characters
    :return: the state of the board from the perspective of player_id, in the format 
...
async def flip(board: Board, player_id: str, row: int, column: int) -> str:
    ...
    :param board: a Memory Scramble board
    :param player_id: ID of player making the flip; 
                     must be a nonempty string of alphanumeric or underscore characters
    ...
    :return: the state of the board after the flip from the perspective of player_id, in the 
             format described in the ps4 handout
    :raises: ValueError if the flip operation fails as described in the ps4 handout.
    """
...
async def map_cards(board: Board, player_id: str, f: Callable[[str], Awaitable[str]]) -> str:
    ...
    :param player_id: ID of player applying the map; 
                     must be a nonempty string of alphanumeric or underscore characters
    :param f: mathematical function from cards to cards (async)
    :return: the state of the board after the replacement from the perspective of player_id,
...
async def watch(board: Board, player_id: str) -> str:
    ...
    :param board: a Memory Scramble board
    :param player_id: ID of player watching the board; 
                     must be a nonempty string of alphanumeric or underscore characters
    :return: the updated state of the board from the perspective of player_id, in the 
             format described in the ps4 handout
    """
